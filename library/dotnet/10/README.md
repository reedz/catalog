...
This directory contains the [ASP.NET Core 10](https://dotnet.microsoft.com/) runtime on Unikraft, in binary compatibility mode.
It implements a simple HTTP server running on Unikraft that provides a simple response to each request.

The application uses AOT (Ahead-Of-Time) compilation for improved performance and reduced startup time.
...
```bash
kraft run --rm -M 1024M -p 8080:8080 --plat qemu --arch x86_64 unikraft.org/dotnet:10
```
...
upbeat_jenny  project://dotnet:qemu/x86_64 /usr/src/SimpleHttp[...]  5 seconds ago  running  1020M  0.0.0.0:8080->8080/tcp  qemu/x86_64
```
